...
The Preview Chooser node is designed to present a set of images to the user, allowing them to visually inspect and select one or more images to continue the workflow. This node integrates a user interaction step into the data processing pipeline, assisting decision-making based on visual information.

## Input types
### Required
- **`mode`**
    - The 'mode' parameter determines the node's operation mode, affecting how user selections are processed and the node's behavior in different scenarios.
    - Comfy dtype: `COMBO[STRING]`
    - Python dtype: `str`
- **`count`**
    - The 'count' parameter specifies the number of images to select or process, allowing control over output quantity based on user input or predefined settings.
    - Comfy dtype: `INT`
    - Python dtype: `int`
### Optional
- **`images`**
    - The 'images' parameter represents the collection of images presented to the user for selection. It plays a crucial role in node operation by providing the visual content for user interaction and selection.
    - Comfy dtype: `IMAGE`
    - Python dtype: `torch.Tensor`
- **`latents`**
    - The 'latents' parameter refers to the latent representations associated with the images. It is used to pass latent information of selected images within the workflow, facilitating further processing or analysis.
    - Comfy dtype: `LATENT`
    - Python dtype: `torch.Tensor`
- **`masks`**
    - The 'masks' parameter provides optional mask information for images, which can be used to focus on specific parts of images in advanced selection or processing scenarios.
    - Comfy dtype: `MASK`
...
- **`masks`**
    - Comfy dtype: `MASK`
    - Returns the mask information of the selected images (if applicable).
    - Python dtype: `Optional[torch.Tensor]`
- **`selected`**
    - Comfy dtype: `STRING`
    - Provides a string or identifier of the selected images, facilitating tracking and further processing.
    - Python dtype: `str`
