...
The Intrinsic_lora_sampling node integrates intrinsic LoRA (Low-Rank Adaptations) into a given model to perform specialized sampling tasks, such as generating depth maps, surface normals, albedo, and shading. It leverages intrinsic LoRA to modify the model's behavior for specific visual tasks, enabling the generation of images with detailed properties based on the selected task.

# Input types
## Required
- **`model`**
    - The model to which the intrinsic LoRA will be applied, used to modify its behavior for specific visual tasks.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- **`lora_name`**
    - The name of the intrinsic LoRA to load and apply to the model, selected from a predefined list of available LoRAs.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`task`**
    - Specifies the visual task for generating images, such as depth maps, surface normals, albedo, or shading, influencing the modifications applied to the model.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`text`**
    ...
    - Python dtype: str
- **`clip`**
    - The CLIP model used for text encoding, assisting the generation process by providing text-based guidance.
    - Comfy dtype: CLIP
    - Python dtype: torch.nn.Module
- **`vae`**
    - The VAE model used to decode the sampled latent representations into images.
    - Comfy dtype: VAE
    - Python dtype: torch.nn.Module
- **`per_batch`**
...
## Optional
- **`image`**
    - Optional image input, used for tasks that require an initial image, providing a starting point for the generation process.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- **`optional_latent`**
    - Optional latent representation, which can serve as a starting point for the generation process, eliminating the need for an initial image.
    - Comfy dtype: LATENT
    - Python dtype: Dict[str, torch.Tensor]

# Output types
- **`image`**
    - The generated image with properties specified by the task, such as depth maps or surface normals.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- **`latent`**
    - The latent representation of the generated image, providing insight into the model's internal representations.
    - Comfy dtype: LATENT
    - Python dtype: Dict[str, torch.Tensor]
