...
The LLMUnstructuredReader node is designed to read and process unstructured files, converting them into a format suitable for indexing and further analysis by the llama_index system. It supports multiple file types and serves as a versatile tool for transforming raw data into structured document formats.

# Input types
## Required
- path
    - Specifies the file path of the unstructured file to be read. This is essential for locating and accessing the file to be processed.
    - Comfy dtype: STRING
    - Python dtype: str
- split_documents
    - A boolean flag indicating whether the input file should be split into multiple documents based on specific criteria. This affects how the data is segmented and structured.
    - Comfy dtype: COMBO[BOOLEAN]
    - Python dtype: bool
## Optional
- extra_info
    - An optional string containing additional information or parameters in JSON format, which can be used to customize the reading process.
    - Comfy dtype: STRING
    - Python dtype: str

# Output types
- documents
    - Processed data from the unstructured file, constructed as a document or list of documents, ready for indexing and analysis.
