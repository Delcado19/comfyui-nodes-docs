...
The Prepare Image Plus node is designed to configure and apply a series of image preprocessing settings, including resizing, cropping, padding, interpolation, sharpening, and flipping. It allows detailed customization of how images are processed, ensuring they are optimally prepared for subsequent operations such as machine learning model input or visual presentation.

# Input types
## Required
- image
    - The primary image input for processing. This parameter is critical as it specifies the image to which all preprocessing settings will be applied.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- target_w
    - Specifies the target width of the image after resizing, affecting the final dimensions of the processed image.
    - Comfy dtype: INT
    - Python dtype: int
- target_h
    - Specifies the target height of the image after resizing, affecting the final dimensions of the processed image.
    - Comfy dtype: INT
    - Python dtype: int
- offset_w
    - Determines the horizontal offset applied to the image, which can adjust the position or alignment of the image.
    - Comfy dtype: INT
    - Python dtype: int
- offset_h
    - Determines the vertical offset applied to the image, affecting the position or alignment of the image in the vertical direction.
    - Comfy dtype: INT
    - Python dtype: int
- crop_left
    - Specifies the amount to crop from the left side of the image, used to remove unwanted edges or for composition.
    - Comfy dtype: INT
    - Python dtype: int
- crop_right
    - Specifies the amount to crop from the right side of the image, helpful for composition or removing excess parts.
    - Comfy dtype: INT
    - Python dtype: int
- crop_top
    - Determines the amount to crop from the top of the image, which can help adjust composition or focus on specific parts of the image.
    - Comfy dtype: INT
    - Python dtype: int
- crop_bottom
    - Determines the amount to crop from the bottom of the image, used for composition or removing unnecessary parts.
    - Comfy dtype: INT
    - Python dtype: int
- padding_left
    ...
    - Python dtype: int
- padding_right
    - Specifies the amount of padding added to the right side of the image, helpful for alignment or visual design.
    - Comfy dtype: INT
    - Python dtype: int
- padding_top
    - Determines the amount of padding added to the top of the image, which can be used for alignment or achieving specific visual effects.
    - Comfy dtype: INT
    - Python dtype: int
- padding_bottom
    - Determines the amount of padding added to the bottom of the image, used for alignment or achieving the desired visual layout.
    - Comfy dtype: INT
    - Python dtype: int
- interpolation
    - Defines the method used for resizing the image, affecting the quality and appearance of the resized image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- sharpening
    - Specifies the degree of sharpening applied to the image, enhancing edge definition and detail.
    - Comfy dtype: FLOAT
    - Python dtype: float
- resize_type
    - Determines the resizing method applied to the image, such as stretching or cropping to fit the specified dimensions.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- flip
    - Indicates whether the image should be flipped horizontally, vertically, or not flipped at all, affecting the orientation of the image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
...
- IMAGE
    - Comfy dtype: IMAGE
    - The output is the image after applying the specified preprocessing settings including resizing, cropping, padding, interpolation, sharpening, and flipping.
    - Python dtype: torch.Tensor
