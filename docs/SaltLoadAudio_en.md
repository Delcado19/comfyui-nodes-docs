...
This node loads audio files into the system, preparing them for subsequent processing or analysis. It is the initial step in the audio processing pipeline, enabling downstream nodes to manipulate or analyze the loaded audio data.

# Input types
## Required
- file_path
    - Specifies the path to the audio file to load. Essential for locating and accessing the audio data to be processed.
    - Comfy dtype: STRING
    - Python dtype: str
- start_seconds
    - Defines the starting time point (in seconds) for audio loading. This allows partial loading of the audio file, facilitating targeted analysis or manipulation.
    - Comfy dtype: FLOAT
    - Python dtype: float
- manual_bpm
    - Allows manual specification of beats per minute (BPM), which can override automatic BPM detection. Useful for audio processing that relies on precise tempo information.
    - Comfy dtype: FLOAT
    - Python dtype: float
- frame_rate
    - Sets the frame rate for audio processing. This parameter affects the temporal resolution of audio analysis and manipulation.
    - Comfy dtype: INT
    - Python dtype: int
## Optional
- duration_seconds
    - Determines the duration (in seconds) of audio to load from the starting point. This enables selective processing of audio segments.
    - Comfy dtype: FLOAT
    - Python dtype: float
...
- bpm
    ...
    - Python dtype: float
- frame_rate
    - The frame rate used during audio loading.
    - Comfy dtype: INT
    - Python dtype: int
- frame_count
    - The total number of frames in the loaded audio segment, calculated based on duration and frame rate.
    - Comfy dtype: INT
    - Python dtype: int
