...
The Film Interpolation (mtb) node uses Google Research's FILM technology for frame interpolation, specifically designed to handle large motions between frames. It provides a method to insert additional frames between a series of images, enhancing the fluidity and smoothness of video content.

## Input types
### Required
- **`images`**
    - The image tensor to be interpolated. This input is critical for determining between which frames interpolation occurs, directly affecting the visual continuity of the output.
    - Comfy dtype: `IMAGE`
...
- **`interpolate`**
    ...
    - Python dtype: `int`
- **`film_model`**
    - The FILM model instance used for interpolation. This model is responsible for the actual computation of interpolated frames, leveraging advanced techniques to effectively manage large motions.
...
- **`image`**
    - Comfy dtype: `IMAGE`
    - The output is a tensor of interpolated images, with additional frames added to create smoother transitions between the original images.
    - Python dtype: `torch.Tensor`
