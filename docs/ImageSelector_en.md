...
The ImageSelector node is designed to filter and select specific images from a given set based on user-defined indices. It supports selecting individual images or ranges of images, facilitating custom image processing and manipulation.

# Input types
## Required
- images
    - The 'images' parameter represents the collection of images used for selection. It is critical for determining the subset of images to process or analyze.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- selected_indexes
    - The 'selected_indexes' parameter specifies the indices of images to select, supporting both individual indices and ranges for flexible image selection. This parameter is case-sensitive.
    - Comfy dtype: STRING
    - Python dtype: str

# Output types
- image
    - The output is a filtered set of images selected according to the specified indices.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
