...
The Seed node manages and generates random seeds to ensure processes that require randomization are repeatable and unique. It can return a predefined seed, generate a new random seed, or modify the seed based on specific conditions, and it also handles metadata updates related to seed information.

# Input types
## Required
- seed
    - The seed parameter is crucial for determining the starting point of randomness. It can use a predefined seed or trigger the generation of a new random seed, thereby influencing the node's output and ensuring result repeatability.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- SEED
    - The seed used or generated during output node execution, which is essential for ensuring repeatable randomness in the process.
    - Comfy dtype: INT
    - Python dtype: int
