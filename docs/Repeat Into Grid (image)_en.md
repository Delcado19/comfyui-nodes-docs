...
The Repeat Into Grid (image) node tiles input image samples into a configurable-dimension grid, effectively repeating the image across specified rows and columns to create a larger, grid-like composite image.

# Input types
## Required
- image
    - The input image to be tiled in the grid. It determines the base image that will be repeated across the specified grid dimensions.
    - Comfy dtype: IMAGE
