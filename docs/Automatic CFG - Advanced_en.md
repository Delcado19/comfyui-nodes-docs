...
The Automatic CFG - Advanced node represents an advanced dynamic Control Flow Graph (CFG) adjustment configuration, designed to improve model performance and flexibility by applying complex CFG modifications.

# Input types
## Required
- **`model`**
    - This is the model to which advanced dynamic CFG adjustments will be applied, serving as the foundation for enhancement.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- **`automatic_cfg`**
    - Specifies the level of automatic CFG adjustment to apply, ranging from basic to advanced configurations.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`skip_uncond`**
    - Determines whether to skip the unconditional generation step, optimizing model performance for specific tasks.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- **`uncond_sigma_start`**
    - The starting sigma value for unconditional generation, one of the fine-tuning parameters for model optimization.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`uncond_sigma_end`**
    - The ending sigma value for unconditional generation, marking the end of the fine-tuning process for model optimization.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`lerp_uncond`**
    - Indicates whether to use linear interpolation for unconditional generation, enhancing model adaptability.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
...
- **`lerp_uncond_sigma_start`**
    - The starting sigma value for linear interpolation in unconditional generation, setting the initial conditions for interpolation.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`lerp_uncond_sigma_end`**
    - The ending sigma value for linear interpolation in unconditional generation, determining the final conditions for interpolation.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`subtract_latent_mean`**
    - Determines whether to subtract the mean of the latent space, affecting the model's generation process.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- **`subtract_latent_mean_sigma_start`**
    - The starting sigma value for subtracting the latent mean, influencing the start of the adjustment.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`subtract_latent_mean_sigma_end`**
    - The ending sigma value for subtracting the latent mean, marking the end of the adjustment.
    - Comfy dtype: FLOAT
    - Python dtype: float
...
- **`latent_intensity_rescale_method`**
    - Specifies the method used to rescale the latent space intensity, affecting model behavior.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`latent_intensity_rescale_cfg`**
    - The configuration value for rescaled latent intensity, determining the scale of the adjustment.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`latent_intensity_rescale_sigma_start`**
    - The starting sigma value for rescaled latent intensity, setting the initial parameters for rescaling.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`latent_intensity_rescale_sigma_end`**
    - The ending sigma value for rescaled latent intensity, determining the final parameters of the adjustment.
    - Comfy dtype: FLOAT
    - Python dtype: float
