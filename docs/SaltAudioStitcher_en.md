...
The SaltAudioStitcher node is designed to stitch two audio files together, with an optional silent transition between them. It enables seamless audio experiences by combining multiple audio clips.
...
- audio_a
    ...
    - Python dtype: bytes
- audio_b
    - The second audio file to be stitched. It follows immediately after the first audio file in the stitched audio output.
    - Comfy dtype: AUDIO
    - Python dtype: bytes
## Optional
- silent_transition_seconds
    - The duration of silence (in seconds) inserted between the two audio files. This allows for smooth transitions or pauses between audio clips.
    - Comfy dtype: FLOAT
    - Python dtype: float

# Output types
- stitched_audio
    - The resulting audio file after stitching the input audio files together, potentially containing silent transitions between the audio segments.
    - Comfy dtype: AUDIO
    - Python dtype: bytes
