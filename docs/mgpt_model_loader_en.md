...
This node loads and initializes the MotionGPT model with specified configuration and model weights. It ensures the model is ready to generate or process motion data by setting up the necessary environment, including downloading the model if not available locally, and preparing it for inference.

# Input types
## Required
- model
    - Specifies the model to load and initialize for motion generation or processing. This parameter determines which specific MotionGPT model configuration and weights to use, affecting the quality and type of motion data the node can generate or process.
