...
The FL_ImageRandomizer node is designed to select and process images from a specified directory, offering options for either random selection or sequential access. It enhances image processing capabilities in workflows by providing a flexible approach to handling image datasets, including randomization for diverse output generation.

# Input types
## Required
- directory_path
    - The filesystem path to the directory containing images. This is essential for locating and loading images to be processed.
    - Comfy dtype: STRING
    - Python dtype: str
- randomize
    - A boolean switch to decide whether to select images randomly or sequentially. This affects the diversity and unpredictability of the output.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- run_trigger
    - A dummy input used to trigger node execution, helping to bypass caching issues.
    - Comfy dtype: INT
    - Python dtype: int
...
- path
    - The filesystem path of the selected image, providing context information or for subsequent operations.
    - Comfy dtype: PATH
    - Python dtype: str
