...
The Revision Settings Pipe (JPS) node is designed to process and adjust image revision settings, facilitating customization of image processing parameters such as cropping, interpolation, and noise augmentation. It serves as a configurable pipeline component in image processing workflows, enabling precise control over revision effects applied to images.

# Input types
## Required
- revision_settings
    - Specifies the settings for image revision, including resolution, interpolation method, cropping preference, offset, strength, and noise augmentation level. This input is critical for determining how the image is modified during the revision process.
...
- rev1_crop
    - Comfy dtype: COMBO[STRING]
    - Specifies the cropping preference for the first revision.
    - Python dtype: str
- rev1_offset
    - Comfy dtype: INT
...
- rev2_crop
    - Comfy dtype: COMBO[STRING]
    - Specifies the cropping preference for the second revision.
    - Python dtype: str
- rev2_offset
    - Comfy dtype: INT
...
- rev1_noiseaug
    - Comfy dtype: FLOAT
    - The noise augmentation level applied during the first revision.
    - Python dtype: float
- rev2_noiseaug
    - Comfy dtype: FLOAT
    - The noise augmentation level applied during the second revision.
    - Python dtype: float
