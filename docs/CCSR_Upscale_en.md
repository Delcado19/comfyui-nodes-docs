...
The CCSR_Upscale node aims to increase the resolution of images or latent representations using advanced upscaling techniques. It employs custom algorithms and models to enlarge images, enhancing quality and detail, with the goal of achieving higher-fidelity output compared to traditional upscaling methods.

# Input types
## Required
- ccsr_model
    - Specify the model used for the upscaling process, which determines the technique and its effectiveness.
    - Comfy dtype: CCSRMODEL
    - Python dtype: str
- image
    - Provide the image to be upscaled, serving as the primary input for the process.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- resize_method
    - Define the method for resizing the image, affecting upscaling quality and characteristics.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- scale_by
    - Set the scaling factor for the upscaling process, influencing the final output size.
    - Comfy dtype: FLOAT
    - Python dtype: float
- steps
    - Indicate the number of steps to execute during upscaling, impacting detail and quality of the enlarged image.
    - Comfy dtype: INT
    - Python dtype: int
- t_max
    - Choose the maximum sampling temperature, affecting randomness and detail in the upscaled image.
    - Comfy dtype: FLOAT
    - Python dtype: float
- t_min
    - Choose the minimum sampling temperature, setting a lower bound for randomness and detail in the upscaled image.
    - Comfy dtype: FLOAT
    - Python dtype: float
- sampling_method
    - Determine the sampling strategy used during upscaling, influencing texture and quality of the output.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- tile_size
    - Set the tile size used in the upscaling process, affecting processing efficiency and detail capture.
    - Comfy dtype: INT
    - Python dtype: int
- tile_stride
    - Define the tile stride during upscaling, influencing overlap between tiles and continuity of detail.
    - Comfy dtype: INT
    - Python dtype: int
- vae_tile_size_encode
    - Specify the tile size for VAE encoding steps, affecting detail retention during encoding.
    - Comfy dtype: INT
    - Python dtype: int
- vae_tile_size_decode
    - Specify the tile size for VAE decoding steps, affecting detail reconstruction during decoding.
    - Comfy dtype: INT
    - Python dtype: int
- color_fix_type
    - Choose the method for color correction, crucial for maintaining color accuracy in the upscaled image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- keep_model_loaded
    - Indicate whether the upscaling model should remain loaded between multiple calls, affecting processing speed and resource usage.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- seed
    - Provide a random seed for the upscaling process, ensuring reproducibility of results.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- upscaled_image
    - Output of the upscaling process, delivering a higher-resolution image with improved quality and detail.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
