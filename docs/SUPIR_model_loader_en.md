...
The SUPIR_model_loader node is responsible for loading the SUPIR model, a key component in the SUPIR framework for image processing and enhancement. It handles model initialization and configuration, ensuring the model is ready for subsequent image processing tasks.

# Input types
## Required
- supir_model
    - Specifies the path to the SUPIR model checkpoint file, essential for loading the model's state for image processing tasks.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: List[str]
- sdxl_model
    - Specifies the path to SDXL model checkpoint files, which are merged with the SUPIR model to enhance its capabilities.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: List[str]
- fp8_unet
    - A flag to determine whether to convert UNet weights to a lower precision format to save VRAM, which may have a slight impact on quality.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- diffusion_dtype
    - Specifies the data type for diffusion operations, providing options for performance optimization or compatibility.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str

# Output types
- SUPIR_model
    - The loaded and configured SUPIR model, ready for immediate use in image processing tasks.
    - Comfy dtype: SUPIRMODEL
    - Python dtype: torch.nn.Module
- SUPIR_VAE
    - The VAE component of the SUPIR model, essential for certain image processing operations.
