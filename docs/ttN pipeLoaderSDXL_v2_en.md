...
This node is used to load and initialize the Stable Diffusion XL model for image generation tasks, providing an enhanced version optimized for large-scale operations.

# Input types
## Required
- ckpt_name
    - Specifies the checkpoint name of the Stable Diffusion XL model, crucial for loading the correct model version for image generation.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- config_name
    - Defines the configuration name, crucial for setting up the model with appropriate parameters and optimization settings.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- vae_name
    - Names the VAE to use with the Stable Diffusion XL model, important for the image generation process and quality.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- clip_skip
    - Determines the number of layers to skip in the CLIP model, affecting the integration of text guidance.
    - Comfy dtype: INT
    - Python dtype: int
- loras
    - Specifies the LoRA modifications to apply, enhancing model capabilities through additional parameters.
    - Comfy dtype: STRING
    - Python dtype: str
- refiner_ckpt_name
    - Names the checkpoint of the refiner model, used to refine or alter generated images.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- refiner_config_name
    - Comfy dtype: COMBO[STRING]
...
# Output types
- sdxl_pipe
    - Provides the initialized Stable Diffusion XL pipeline, usable for image generation tasks.
    - Comfy dtype: PIPE_LINE_SDXL
    - Python dtype: dict
- model
    - Returns the loaded model components of the Stable Diffusion XL pipeline.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- positive
    - Outputs the positive conditioning component, guiding image generation toward desired subjects.
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- negative
    - Outputs the negative conditioning component, guiding image generation away from unwanted subjects.
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- vae
    - Returns the loaded VAE component, crucial for image encoding and decoding processes.
    - Comfy dtype: VAE
    - Python dtype: torch.nn.Module
- clip
    - Outputs the loaded CLIP model component, used for text guidance in image generation.
    - Comfy dtype: CLIP
    - Python dtype: torch.nn.Module
- refiner_model
    - Provides the loaded refiner model component, used to refine or alter generated images.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- refiner_positive
    - Comfy dtype: CONDITIONING
