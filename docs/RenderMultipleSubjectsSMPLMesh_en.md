...
This node is designed to render multiple SMPL mesh subjects in a scene. It leverages 3D mesh rendering techniques to simultaneously visualize the motion and morphology of multiple subjects, providing a comprehensive view of their spatial interactions and dynamics.

# Input types
## Required
- smpl_multi_subjects
    - This input is critical for rendering, as it contains the SMPL models of multiple subjects, including their mesh data and potentially motion information. This dataset enables the node to accurately visualize each subject in the scene.
    - Comfy dtype: SMPL_MULTIPLE_SUBJECTS
    - Python dtype: List[Dict[str, Union[np.ndarray, List[np.ndarray]]]]
- draw_platform
    - Specifies whether to draw a standing platform for the subjects, enhancing the realism of the visualization.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- depth_only
    ...
    - Python dtype: bool
- fx_offset
    - Horizontal offset of the camera focus, allowing adjustment of the rendered scene's perspective.
    - Comfy dtype: FLOAT
    - Python dtype: float
- fy_offset
    - Vertical offset of the camera focus, allowing fine-tuning of the scene's visual perspective.
    - Comfy dtype: FLOAT
    - Python dtype: float
- move_x
    - Moves the camera along the x-axis, providing control over scene composition.
    - Comfy dtype: FLOAT
    - Python dtype: float
- move_y
    - Moves the camera along the y-axis, allowing adjustment of the scene's framing.
    - Comfy dtype: FLOAT
    - Python dtype: float
- move_z
    - Moves the camera along the z-axis, providing the ability to change the scene's depth.
    - Comfy dtype: FLOAT
    - Python dtype: float
- rotate_x
    - Rotates the camera around the x-axis, enabling changes to the scene's vertical orientation.
    - Comfy dtype: FLOAT
    - Python dtype: float
- rotate_y
    - Rotates the camera around the y-axis, allowing changes to the scene's horizontal orientation.
    - Comfy dtype: FLOAT
    - Python dtype: float
- rotate_z
    - Rotates the camera around the z-axis, providing the possibility to adjust the scene's rotational perspective.
    - Comfy dtype: FLOAT
    - Python dtype: float
- background_hex_color
    - Sets the background color of the scene using a hex color code, enhancing visual appeal.
    - Comfy dtype: STRING
    - Python dtype: str

## Optional
- normals
    - Determines whether to render mesh normals, which can provide additional detail for visual analysis.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- remove_background
...
- IMAGE
    - Comfy dtype: IMAGE
    - The output is a visual representation of multiple SMPL models, rendered as meshes in the scene. It provides a detailed view of the subjects' morphology and motion.
    - Python dtype: np.ndarray
- DEPTH_MAP
    - Comfy dtype: IMAGE
    - Renders the depth map of the scene, providing insight into the spatial relationships and distances between subjects.
    - Python dtype: np.ndarray
