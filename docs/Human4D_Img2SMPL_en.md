...
The Human4D_Img2SMPL node converts 2D human images into 3D representations using the SMPL model. It employs a deep‑learning model to detect human shape in the image, estimate pose, and generate a corresponding 3D mesh, enabling advanced motion analysis and visualization.

# Input types
## Required
- human4d_model
    - The human4D model packages the necessary configuration and models to detect a human in an image and generate its 3D SMPL representation. It plays a crucial role in the node’s accurate handling and conversion of 2D images to 3D models.
    - Comfy dtype: HUMAN4D_MODEL
    - Python dtype: SimpleNamespace
- image
    - The input tensor containing the human images to be converted to 3D SMPL models. This tensor is essential for the node to perform human detection and pose estimation.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- det_confidence_thresh
    - Confidence threshold for human detection. This parameter filters out low‑confidence detections, ensuring only high‑confidence human images are processed into 3D models.
    - Comfy dtype: FLOAT
    - Python dtype: float
- det_iou_thresh
    - Intersection‑over‑Union (IOU) threshold for human detection. It manages overlap between detected bounding boxes, improving detection precision.
    - Comfy dtype: FLOAT
    - Python dtype: float
- det_batch_size
    - Batch size for processing detections. This parameter affects the throughput and efficiency of the human detection process, balancing speed and memory usage.
    - Comfy dtype: INT
    - Python dtype: int
- hmr_batch_size
    - Batch size for the HMR (Human Mesh Recovery) process. It determines how many human images are processed simultaneously for 3D modeling, affecting node performance and resource usage.
    - Comfy dtype: INT
    - Python dtype: int

## Optional
- opt_scorehmr_refiner
    - Optional parameter to optimize the HMR process score. If provided, it can improve the accuracy of the 3D SMPL models generated by the node.
