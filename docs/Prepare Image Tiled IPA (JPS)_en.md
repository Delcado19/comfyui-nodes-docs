...
Prepare Image Tiled IPA (JPS) node configures and applies image preparation settings for block image processing. It adjusts parameters such as model type, weight type, noise level, and image tiling options based on input specifications to enable customized preprocessing, thereby enhancing image synthesis or processing tasks.

# Input types
## Required
- image
- Specify the image to process, serving as the basis for all subsequent image preparation operations.
    - Comfy dtype: IMAGE
    - Python dtype: IMAGE
- target_w
- Set the target width for image processing, affecting the output image size.
    - Comfy dtype: INT
    - Python dtype: int
- target_h
- Define the target height for image processing, affecting the output image size.
    - Comfy dtype: INT
    - Python dtype: int
- zoom
- Determine the scaling level applied to the image, affecting the proportion of image detail.
    - Comfy dtype: INT
    - Python dtype: int
- offset_w
- Specify the horizontal offset of the image, adjusting its position along the width.
    - Comfy dtype: INT
    - Python dtype: int
- offset_h
- Specify the vertical offset of the image, adjusting its position along the height.
    - Comfy dtype: INT
    - Python dtype: int
- interpolation
- Choose the interpolation method for image scaling, affecting the smoothness and quality of the scaled image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- sharpening
- Set the sharpening level applied to the image, enhancing edge definition and detail.
    - Comfy dtype: FLOAT
    - Python dtype: float
- tile_short
- Define the base length of the shortest side for tiling, affecting the tiling pattern and size.
    - Comfy dtype: INT
    - Python dtype: int
- prepare_type
- Select the preparation type for the image, determining the specific processing method to apply.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- IMAGE
- Return the processed image, with tiling, interpolation, and sharpening applied, ready for further use or analysis.
    - Comfy dtype: IMAGE
    - Python dtype: IMAGE
