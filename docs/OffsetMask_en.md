...
The OffsetMask node is designed to manipulate and transform masks by applying specified offsets, rotation, and duplication factors. It is capable of creating multiple variants of a given mask, allowing dynamic adjustments to position and orientation, which is particularly useful in image processing and enhancement tasks.

# Input types
## Required
- mask
    - The input mask or batch of masks to be transformed. This parameter is the core of the node's operation, serving as the foundation for all subsequent transformations.
    - Comfy dtype: MASK
    - Python dtype: torch.Tensor
- x
    - Specifies the horizontal offset to be applied to the mask. This parameter controls the lateral displacement of the mask.
    - Comfy dtype: INT
    - Python dtype: int
- y
    - Specifies the vertical offset to be applied to the mask. This parameter controls the longitudinal displacement of the mask.
    - Comfy dtype: INT
    - Python dtype: int
- angle
    - Defines the angle (in degrees) by which the mask is rotated. This parameter allows rotational adjustments to the mask.
    - Comfy dtype: INT
    - Python dtype: int
- duplication_factor
    - The number of times the mask is duplicated to form a batch. This parameter allows the creation of multiple mask variants from a single input.
    - Comfy dtype: INT
    - Python dtype: int
- roll
    - Determines whether edge wrapping is applied during the offset process. This boolean parameter affects how the mask behaves at its boundaries.
    - Comfy dtype: BOOLEAN
    - Python dtype: bool
- incremental
    ...
    - Python dtype: bool
- padding_mode
    - Specifies the padding mode to be used when transforming the mask. This parameter affects how the edges of the mask are handled during the offset process.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str

# Output types
- mask
    - The transformed mask or batch of masks after applying the specified offsets, rotation, and duplication factors.
    - Comfy dtype: MASK
    - Python dtype: torch.Tensor
