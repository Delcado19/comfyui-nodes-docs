...
The easy humanSegmentation node is designed to segment human silhouettes from images. It leverages advanced machine learning models to accurately identify and separate human subjects from the background. This node is essential for applications requiring precise human silhouettes, such as photo editing, augmented reality, and various forms of digital content creation.

# Input types
## Required
- image
    - The image parameter is the input image that the node will process to segment human silhouettes. It plays a crucial role in determining the accuracy and quality of the segmentation output.
    - Comfy dtype: IMAGE
    - Python dtype: Image.Image
- method
    - The method parameter specifies the segmentation technique or model to be used. This choice affects segmentation accuracy, performance, and suitability for different types of images or requirements.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- confidence
    - The confidence parameter allows setting a threshold for segmentation confidence, filtering out less certain segments to ensure higher accuracy in results.
    - Comfy dtype: FLOAT
    - Python dtype: float

# Output types
- image
    - The image output provides the segmented image with human silhouettes separated from the background, ready for further processing or visualization.
    - Comfy dtype: IMAGE
    - Python dtype: Image.Image
- mask
    - The mask output provides a binary mask indicating the regions in the image corresponding to human silhouettes, suitable for various image editing and processing tasks.
    - Comfy dtype: MASK
    - Python dtype: torch.Tensor
- ui
    - The ui parameter represents the user interface component displaying segmentation results, providing a visualization of human silhouettes separated from the background.
