...
This node aims to generate a morphing animation between a series of images stored in a specified directory. It creates a GIF animation that smoothly transitions from one image to another over a given number of frames, allowing customization of transition speed, loop count, and output size. The node facilitates the creation of dynamic visual content by automating the morphing process.

# Input types
## Required
- transition_frames
    - Specifies the number of frames used for the transition between each pair of images, affecting the smoothness of the morphing animation.
    - Comfy dtype: INT
    - Python dtype: int
- still_image_delay_ms
    - Determines the delay in milliseconds before each still image transitions, allowing a pause on each image in the sequence.
    - Comfy dtype: FLOAT
    - Python dtype: int
- duration_ms
    - Sets the duration (in milliseconds) of each frame in the animation, controlling the speed of the morphing effect.
    - Comfy dtype: FLOAT
    - Python dtype: float
- loops
    ...
    - Python dtype: int
- max_size
    - Limits the maximum size of images in the animation, ensuring the output file remains within a manageable size range.
    - Comfy dtype: INT
    - Python dtype: int
- input_path
    - The directory path where input images are stored, serving as the source for the morphing animation.
    - Comfy dtype: STRING
    - Python dtype: str
- input_pattern
    - A pattern used to match filenames in the input directory, allowing selective inclusion of images in the animation.
    - Comfy dtype: STRING
    - Python dtype: str
- output_path
    ...
    - Python dtype: str
- filename
    - The name of the output file, excluding the extension, allowing customization of the output filename.
    - Comfy dtype: STRING
    - Python dtype: str
- filetype
...
# Output types
- filepath_text
    - The absolute path of the saved output file, providing a direct link to the generated animation.
    - Comfy dtype: STRING
    - Python dtype: str
- filename_text
    - The name of the saved output file, including its extension, providing a clear identifier for the generated animation.
    - Comfy dtype: STRING
    - Python dtype: str
