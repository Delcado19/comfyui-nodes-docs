...
This node aims to simplify the process of exporting SMPL-format 3D models of multiple subjects to various 3D software environments. Its goal is to optimize the workflow of integrating complex multi-subject 3D models into professional 3D CGI applications, thereby improving the productivity of 3D animators and VFX artists.

# Input types
## Required
- smpl_multi_subjects
    - A dataset containing SMPL-format 3D models of multiple subjects is key to exporting to 3D software. It plays a crucial role in node operations by providing the data necessary for processing and conversion.
    - Comfy dtype: SMPL_MULTIPLE_SUBJECTS
    - Python dtype: List[Dict[str, Any]]
- foldername_prefix
    - A prefix for the folder name where exported files are stored, facilitating organized storage and easy retrieval.
    - Comfy dtype: STRING
    - Python dtype: str
- format
    - The format in which the 3D model will be exported, such as 'obj', 'fbx', or 'glb', determines compatibility with various 3D software.
