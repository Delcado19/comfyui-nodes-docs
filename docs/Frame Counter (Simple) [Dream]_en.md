...
This node generates a frame counter based on a specified frame index, total frame count, and frames per second. It assists animation production by tracking and managing frame progress.

# Input types
## Required
- frame_index
    - Specifies the current frame index. It is critical for determining the starting point of the frame counter.
    - Comfy dtype: INT
    - Python dtype: int
- total_frames
    ...
    - Python dtype: int
- frames_per_second
    - Determines the frame rate by specifying the number of frames played per second. This affects the timing and speed of the animation.
    - Comfy dtype: INT
    - Python dtype: int
