...
KSampler (Efficient) node aims to sample latent images efficiently using various sampling techniques, optimized for specific model configurations and conditioning parameters. It uses the Comfy KSampler node to generate or refine latent images based on provided seed, steps, and other relevant parameters, improving processing performance without compromising image quality.

# Input types
## Required
- **`model`**
- Specify the model used for sampling, which plays a crucial role in determining the features and quality of the generated latent image.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- **`seed`**
- Provide a seed value to ensure reproducibility of the sampling process, affecting the randomness and variation of the generated image.
    - Comfy dtype: INT
    - Python dtype: int
- **`steps`**
- Define the number of steps to take during sampling, influencing the detail and quality of the output image.
    - Comfy dtype: INT
    - Python dtype: int
- **`cfg`**
- Use a configuration parameter to adjust the sampling process, affecting the creativity and fidelity of the generated image.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`sampler_name`**
- Identify the specific sampling algorithm to use, influencing the method and techniques for generating the latent image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`scheduler`**
- Specify the scheduler for the sampling process, controlling the progression of steps and their impact on image quality.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`positive`**
- Provide positive conditioning text to guide image generation toward desired attributes or themes.
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- **`negative`**
- Provide negative conditioning text to steer image generation away from certain attributes or themes.
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- **`latent_image`**
- Supply an initial latent image to be optimized or modified through the sampling process, serving as the starting point for generation.
    - Comfy dtype: LATENT
    - Python dtype: torch.Tensor
- **`denoise`**
- Use a parameter to control the denoising level during sampling, affecting the clarity and sharpness of the generated image.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`preview_method`**
- Specify the method for previewing the sampling process, influencing how intermediate or final results are visualized.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`vae_decode`**
- Indicate whether to use a VAE model to decode the latent image, affecting the final image representation.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: bool
## Optional
- **`optional_vae`**
- Provide an optional VAE model for decoding, offering flexibility in the image generation process.
    - Comfy dtype: VAE
    - Python dtype: torch.nn.Module or None
- **`script`**
- Include an optional script to customize or extend the sampling process, enabling advanced operations on the generation.
    - Comfy dtype: SCRIPT
    - Python dtype: str

# Output types
- **`MODEL`**
- Use the model during sampling, which may have been modified or updated.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- **`CONDITIONING+`**
- Use or generate positive conditioning information during sampling.
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- **`CONDITIONING-`**
- Use or generate negative conditioning information during sampling.
    - Comfy dtype: CONDITIONING
    - Python dtype: str
- **`LATENT`**
- Output the latent image generated or optimized by the sampling process, usable for further processing or conversion to a visible image.
    - Comfy dtype: LATENT
    - Python dtype: torch.Tensor
- **`VAE`**
- Reference or use a VAE model during sampling, if applicable.
    - Comfy dtype: VAE
    - Python dtype: torch.nn.Module
- **`IMAGE`**
- Produce the final or intermediate image during sampling, determined by the preview method and other parameters.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
