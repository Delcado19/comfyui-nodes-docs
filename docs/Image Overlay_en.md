...
The Image Overlay node is used to overlay one image onto another, providing various customization options such as resizing, rotation, opacity adjustment, and optional masking. This node creates composite images by applying transformation and blending techniques to the overlay image, then merging it with the base image.

# Input types
## Required
- base_image
    - The base image onto which the overlay image is applied. It serves as the background of the composite image.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- overlay_image
    - The image to be overlaid onto the base image. This image can be resized, rotated, and have its opacity adjusted before being overlaid.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- overlay_resize
    - Specifies how the overlay image is resized to fit the base image, with options including fitting to dimensions, scaling by a factor, or resizing to specific width and height.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- resize_method
    - The method used to resize the overlay image, affecting the quality and appearance of the resized image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- rescale_factor
    - The scaling factor for the overlay image when the "resize by rescale_factor" option is selected.
    - Comfy dtype: FLOAT
    - Python dtype: float
- width
    - The width of the overlay image after resizing when the "resize to specified width and height" option is selected.
    - Comfy dtype: INT
    - Python dtype: int
- height
    - The height of the overlay image after resizing when the "resize to specified width and height" option is selected.
    - Comfy dtype: INT
    - Python dtype: int
- x_offset
    - The horizontal offset for placing the overlay image on the base image.
    - Comfy dtype: INT
    - Python dtype: int
- y_offset
    - The vertical offset for placing the overlay image on the base image.
    - Comfy dtype: INT
    - Python dtype: int
- rotation
    - The angle (in degrees) by which the overlay image is rotated before being applied to the base image.
    - Comfy dtype: INT
    - Python dtype: float
- opacity
    - The opacity of the overlay image, allowing for the creation of transparent overlay effects.
    - Comfy dtype: FLOAT
    - Python dtype: float
## Optional
- optional_mask
    - An optional mask that can be applied to the overlay image, controlling the opacity of different parts of the overlay image.
    - Comfy dtype: MASK
