...
The LoadPromptsFromFile node reads and parses prompt data from a specified file, extracting structured information based on predefined patterns. Its goal is to facilitate the generation or manipulation of text prompts by providing a structured format for positive and negative prompt components.

# Input types
## Required
- prompt_file
    - Specify the file from which to load prompts, serving as the key input that determines the source of the prompt data.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str

# Output types
- zipped_prompt
    - Return a list of tuples, each containing the positive text, negative text, and the filename of the loaded prompt, representing the structured prompt data extracted from the file.
