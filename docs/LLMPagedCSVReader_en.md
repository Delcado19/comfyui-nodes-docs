...
The LLMPagedCSVReader node is specifically designed to read CSV files and convert them into a structured document format with pagination capabilities. It enables efficient processing and manipulation by splitting large CSV files into manageable pages.

# Input types
## Required
- path
    - Specifies the file path of the CSV file to read. This is essential for locating and accessing the file for reading.
    - Comfy dtype: STRING
    - Python dtype: str
- encoding
...
- delimiter
    ...
    - Python dtype: str
- quotechar
    - Indicates the character used to enclose text containing delimiters, typically a double quote (").
    - Comfy dtype: STRING
    - Python dtype: str
## Optional
- extra_info
    - Allows inclusion of additional optional information that may be relevant to processing the CSV file.
    - Comfy dtype: STRING
    - Python dtype: str

# Output types
- documents
    - Returns a structured document format representing the contents of the CSV file, segmented into multiple pages for easier processing.
