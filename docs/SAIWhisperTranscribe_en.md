...
The SAIWhisperTranscribe node is designed to transcribe audio and video files using the Whisper model. It processes media files to extract audio, applies the Whisper model for transcription, and returns detailed transcription data, including raw text, structured transcription frames, and additional metadata such as frame rate and count.

# Input types
## Required
- **`whisper_model`**
    - The Whisper model, processor, and device configuration used for transcription. This parameter is critical for determining how audio is processed and transcribed, affecting the accuracy and quality of the transcription output.
    - Comfy dtype: WHISPER_MODEL
    - Python dtype: Tuple[torch.nn.Module, Any, torch.device]
- **`file_path`**
    - The path to the media file to be transcribed. This parameter is essential for locating the file and determining its media type, which affects the transcription process.
    - Comfy dtype: STRING
    - Python dtype: str
## Optional
- **`frame_rate`**
    - Specifies the frame rate used when extracting audio from video files. It affects the granularity of transcription timestamps and frames.
    - Comfy dtype: FLOAT
    - Python dtype: float
- **`chunk_type`**
    - Determines the segmentation type for transcription, which can be by sentence or word, affecting the structure of the transcription output.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- **`max_new_tokens`**
    - The maximum number of new tokens generated for the transcription. This parameter controls the length and detail of the transcription output.
    - Comfy dtype: INT
    - Python dtype: int
...
- **`transcription_frame_dict`**
    - A dictionary mapping frame numbers to transcribed text, used for synchronizing the transcription with video frames.
    - Comfy dtype: DICT
    - Python dtype: Dict[int, str]
- **`prompt_schedule`**
    - A structured representation mapping transcribed text to specific frame numbers, formatted as a timeline.
    - Comfy dtype: STRING
    - Python dtype: str
- **`images`**
    - Image tensors extracted or generated during the transcription process, corresponding to video frames or placeholders for audio files.
    - Comfy dtype: IMAGE
...
- **`frame_rate`**
    ...
    - Python dtype: float
- **`frame_count`**
    - The total number of frames in the video file, relevant to video file transcription.
    - Comfy dtype: INT
    - Python dtype: int
