...
The Pixel Deflicker node aims to reduce flicker in image sequences, improving visual consistency between frames. It combines temporal smoothing and adaptive blending techniques to mitigate rapid brightness changes, achieving smoother transitions and higher image quality.

# Input types
## Required
- images
    - A batch of images that require flicker reduction. This input is essential for identifying and alleviating flicker effects in the sequence.
    - Comfy dtype: IMAGE
    - Python dtype: List[torch.Tensor]
- window_size
    - Specify the window size used for temporal smoothing, which affects the degree of flicker reduction across frames.
    - Comfy dtype: INT
    - Python dtype: int
- blending_strength
    - Control the blending intensity between the original and smoothed frames, allowing fine‑tuning of the flicker mitigation effect.
    - Comfy dtype: FLOAT
    - Python dtype: float
- batch_size
    - Determine the number of images processed at once, influencing the node's execution efficiency.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- image
    - Output tensor containing the batch of images after flicker reduction, demonstrating enhanced visual consistency.
    - Comfy dtype: IMAGE
