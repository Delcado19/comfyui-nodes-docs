...
The RectGrabCut node uses the GrabCut algorithm to perform image segmentation on a predefined rectangular region. It aims to separate the foreground from the background within the specified rectangle, thereby enhancing the focus on the desired object or area in the image.

# Input types
## Required
- image
    - The input image to be segmented. This image will undergo foreground and background separation within the specified rectangular region.
    - Comfy dtype: IMAGE
...
- y2
    ...
    - Python dtype: int
- iterations
    - The number of iterations the GrabCut algorithm should run to optimize the segmentation result.
    - Comfy dtype: INT
    - Python dtype: int
- output_format
    - The output format of the segmented image, which affects the post-segmentation processing and display of the image.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str

# Output types
- image
    - The segmented image after foreground and background separation within the specified rectangular region.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
