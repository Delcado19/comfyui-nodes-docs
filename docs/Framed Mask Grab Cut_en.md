...
This node applies the GrabCut algorithm to segment foreground and background in an image, using a frame-based approach to improve segmentation accuracy. It allows specific regions to be designated as foreground or background, thereby improving the accuracy of the generated mask.

# Input types
## Required
- image
    - The input image to which the GrabCut algorithm will be applied, used for segmenting foreground and background.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- thresh
    - The threshold used to determine the initial segmentation of foreground and background.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- iterations
    - The number of iterations the GrabCut algorithm will run, affecting the fineness of the segmentation.
    - Comfy dtype: INT
    - Python dtype: int
- margin
    - The edge size around the frame that is treated as definite background, helping achieve more accurate segmentation.
    - Comfy dtype: INT
    - Python dtype: int
- frame_option
    - Options to include or exclude certain frame regions as background, allowing more control over the segmentation process.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- threshold_FGD
    - The threshold for classifying pixels as definite foreground, aiding in the creation of the initial mask.
    - Comfy dtype: INT
    - Python dtype: float
- threshold_PR_FGD
    - The threshold for classifying pixels as probable foreground, used to refine the segmentation mask.
    - Comfy dtype: INT
    - Python dtype: float
- output_format
    - The format of the output, which can be the segmented mask or the image with background removed.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str

# Output types
- image
    - The output of the node, which is the foreground segmentation mask obtained after applying the GrabCut algorithm.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
