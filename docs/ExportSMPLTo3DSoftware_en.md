...
The ExportSMPLTo3DSoftware node aims to simplify the export process of SMPL models to 3D software, providing users with convenience for integrating these models in various 3D CGI workflows. It abstracts the complexity of the conversion and export process, enabling users to more easily use SMPL models in their preferred 3D environments.

# Input types
## Required
- smpl
    - smpl input specifies the SMPL model to export. This input is critical to the node's operation, directly affecting its ability to process and prepare the model for export to 3D software, thereby impacting the entire workflow and output quality.
    - Comfy dtype: SMPL
    - Python dtype: List[str]
- foldername_prefix
    - foldername_prefix input allows users to specify a prefix for the output folder where exported models are stored, providing a way to organize exported files.
    - Comfy dtype: STRING
    - Python dtype: str
- format
    - format input determines the file format of the exported model, such as 'glb', allowing users to choose a format compatible with their 3D software environment.
