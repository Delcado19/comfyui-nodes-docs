# Documentation
- Class name: ModelMergeMochiPreview
- Category: model/merging/model specific
- Output node: False
- Repo Ref: https://github.com/comfyanonymous/ComfyUI

ModelMergeMochiPreview is a ComfyUI node registered by `comfy_extras.nodes_model_merging_model_specific`. The live metadata did not provide a longer description.

# Input types
## Required
- model1
    - The model1 input is used by this node during execution.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- model2
    - The model2 input is used by this node during execution.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- pos_frequencies.
    - The pos_frequencies. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- t_embedder.
    - The t_embedder. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- t5_y_embedder.
    - The t5_y_embedder. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- t5_yproj.
    - The t5_yproj. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.0.
    - The blocks.0. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.1.
    - The blocks.1. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.2.
    - The blocks.2. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.3.
    - The blocks.3. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.4.
    - The blocks.4. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.5.
    - The blocks.5. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.6.
    - The blocks.6. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.7.
    - The blocks.7. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.8.
    - The blocks.8. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.9.
    - The blocks.9. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.10.
    - The blocks.10. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.11.
    - The blocks.11. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.12.
    - The blocks.12. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.13.
    - The blocks.13. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.14.
    - The blocks.14. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.15.
    - The blocks.15. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.16.
    - The blocks.16. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.17.
    - The blocks.17. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.18.
    - The blocks.18. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.19.
    - The blocks.19. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.20.
    - The blocks.20. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.21.
    - The blocks.21. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.22.
    - The blocks.22. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.23.
    - The blocks.23. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.24.
    - The blocks.24. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.25.
    - The blocks.25. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.26.
    - The blocks.26. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.27.
    - The blocks.27. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.28.
    - The blocks.28. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.29.
    - The blocks.29. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.30.
    - The blocks.30. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.31.
    - The blocks.31. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.32.
    - The blocks.32. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.33.
    - The blocks.33. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.34.
    - The blocks.34. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.35.
    - The blocks.35. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.36.
    - The blocks.36. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.37.
    - The blocks.37. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.38.
    - The blocks.38. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.39.
    - The blocks.39. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.40.
    - The blocks.40. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.41.
    - The blocks.41. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.42.
    - The blocks.42. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.43.
    - The blocks.43. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.44.
    - The blocks.44. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.45.
    - The blocks.45. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.46.
    - The blocks.46. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- blocks.47.
    - The blocks.47. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float
- final_layer.
    - The final_layer. input is used by this node during execution.
    - Comfy dtype: FLOAT
    - Python dtype: float

# Output types
- MODEL
    - The MODEL output is produced by this node.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module

# Usage tips
- Infra type: unknown

# Source code
[View source repository](https://github.com/comfyanonymous/ComfyUI)

*Source code is not embedded in this doc — browse the pack's repository at the link above.*
