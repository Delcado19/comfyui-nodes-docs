...
The SaltLayerScheduler node is used to schedule parallax motion effects in audiovisual presentations, enabling dynamic camera movement to enhance the sense of depth and immersion in the visual experience.
...
- zoom_speed
    ...
    - Python dtype: float
- pan_speed
    - Determines the speed of panning, influencing lateral movement in the parallax motion.
    - Comfy dtype: FLOAT
    - Python dtype: float
- pan_directions
    - Defines the direction of panning, allowing for diverse lateral movement patterns.
    - Comfy dtype: STRING
    - Python dtype: str
- direction_change_frames
    - Indicates the frame at which the panning direction changes, adding dynamism to the movement.
    - Comfy dtype: STRING
    - Python dtype: str
- tremor_scale
    - Sets the scale of the "tremor" effect, adding subtle jitter to the parallax motion to enhance realism.
    - Comfy dtype: FLOAT
    - Python dtype: float
- tremor_octaves
    - Determines the number of octaves for the tremor effect, affecting its complexity and texture.
    - Comfy dtype: INT
    - Python dtype: int
- tremor_persistence
    - Controls the persistence of the tremor effect, influencing its amplitude variation over time.
    - Comfy dtype: FLOAT
    - Python dtype: float
- tremor_lacunarity
    - Adjusts the lacunarity of the tremor effect, affecting the frequency of tremor variations.
    - Comfy dtype: FLOAT
    - Python dtype: float
- direction_curve
    - Specifies the easing function for direction changes, making transitions between panning directions smoother.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: List[str]
- start_x
    - Sets the starting X position of the camera, determining its initial horizontal placement.
    - Comfy dtype: FLOAT
    - Python dtype: float
- start_y
    - Sets the starting Y position of the camera, determining its initial vertical placement.
    - Comfy dtype: FLOAT
    - Python dtype: float
- zoom_mode
    - Selects the zoom mode (zoom in, zoom out, or both), defining the zoom behavior throughout the animation.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: List[str]
- layer_offsets
    - Defines offsets for different layers, helping to create layered depth effects in parallax motion.
    - Comfy dtype: STRING
    - Python dtype: str

# Output types
- schedule_list
    - Represents a list of scheduled parallax motion effects, including scaling, panning, and tremor adjustments for each frame.
    - Comfy dtype: LIST
