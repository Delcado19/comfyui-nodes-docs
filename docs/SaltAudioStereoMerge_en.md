...
The SaltAudioStereoMerge node is designed to merge two mono audio inputs into a single stereo audio output. It ensures both audio inputs are the same length and converts them to mono (if not already) before merging into a stereo track.

# Input types
## Required
- audio_a
    - The first audio input to merge. It is treated as one channel of the resulting stereo audio.
    - Comfy dtype: AUDIO
    - Python dtype: bytes
- audio_b
    - The second audio input to merge with the first input. It complements the first input to form the stereo audio output.
    - Comfy dtype: AUDIO
    - Python dtype: bytes

# Output types
- audio
    - The stereo audio output produced by merging the two input tracks.
    - Comfy dtype: AUDIO
    - Python dtype: bytes
