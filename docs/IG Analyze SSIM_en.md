...
The IG Analyze SSIM node evaluates similarity between consecutive images in a given folder using the Structural Similarity Index Measure (SSIM). It computes SSIM values between consecutive image pairs, potentially indicating changes or variations in visual content. Additionally, it visualizes these SSIM values through plotting, providing a graphical representation of similarity trends across the image sequence.

# Input types
## Required
- folder
    - The 'folder' parameter specifies the directory containing the images to be analyzed. It is crucial for determining the set of images on which SSIM computation and comparison are performed.
    - Comfy dtype: STRING
    - Python dtype: str

## Optional
- ymin
    - The 'ymin' parameter sets the minimum value of the y-axis for the SSIM plot, allowing customization of the vertical scale of the plot for better visualization of SSIM data.
    - Comfy dtype: FLOAT
    - Python dtype: float
- ymax
    - The 'ymax' parameter sets the maximum value of the y-axis for the SSIM plot, enabling adjustment of the vertical scale to best visualize SSIM trends.
    - Comfy dtype: FLOAT
    - Python dtype: float

# Output types
- image
    - The output is an image tensor representing the plot of SSIM data between consecutive images. This graphical output provides a visual summary of structural similarity trends across the image sequence.
    - Comfy dtype: IMAGE
