...
The Image Motion node simulates image motion through a series of transformations, including scaling and translation in the x and y directions, while applying overlapping masks for more complex motion effects.

# Input types
## Required
- image
    - The primary image to which motion effects will be applied. It serves as the foundation for all subsequent transformations.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- zoom
    - Defines the scaling level of the image, allowing zoom-in and zoom-out effects to simulate motion.
    - Comfy dtype: FLOAT
    - Python dtype: float
- mask_i_feather
    - Unknown
...
- y_translation
    ...
    - Python dtype: float
- frame_counter
    - Tracks the number of frames processed, used for time-varying animations or effects.
    - Comfy dtype: FRAME_COUNTER
    - Python dtype: int
## Optional
- noise
    - Optional image used to add noise to the motion effect, enhancing realism.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- output_resize_width
    - Optionally adjusts the width of the output image, affecting the final image dimensions.
    - Comfy dtype: INT
    - Python dtype: int
- output_resize_height
    - Optionally adjusts the height of the output image, affecting the final image dimensions.
    - Comfy dtype: INT
    - Python dtype: int
...
- image
    - Comfy dtype: IMAGE
    - The transformed image after applying motion effects.
    - Python dtype: torch.Tensor
- mask1
    - Comfy dtype: MASK
    - The first mask used to create the motion effect, the result after transformation.
    - Python dtype: torch.Tensor
- mask2
    - Comfy dtype: MASK
    - The second mask applied to enhance the motion effect, the result after transformation.
    - Python dtype: torch.Tensor
- mask3
    - Comfy dtype: MASK
    - The third mask contributing to layered motion effects, the result after transformation.
    - Python dtype: torch.Tensor
