...
The ChameleonMask node aims to dynamically adjust or modify image masks based on specific criteria or inputs, enabling flexible and context-sensitive mask processing.

# Input types
## Required
- dst
- Target image, the location where the mask will be applied, serving as the canvas for the final output.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- src
- Source image, used to generate the mask, the primary input for mask creation.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
- thresh_blur
- Defines the threshold for mask blur, affecting the smoothness of mask edges.
    - Comfy dtype: INT
    - Python dtype: int
- close_dist
- Specifies the distance for the mask closing operation, affecting the compactness of the mask.
    - Comfy dtype: INT
    - Python dtype: int
- open_dist
- Determines the distance for the opening operation, affecting the granularity of the mask.
    - Comfy dtype: INT
    - Python dtype: int
- size_dist
- Controls the adjustment of mask size distribution, affecting the overall mask structure.
    - Comfy dtype: INT
    - Python dtype: int
- mask_blur
- Sets the blur level applied to the mask, modifying its softness and transparency.
    - Comfy dtype: INT
    - Python dtype: int
- contrast_adjust
- Adjusts the mask's contrast, enhancing or reducing its visual intensity.
    - Comfy dtype: FLOAT
    - Python dtype: float
- mode
- Specifies the operation mode for mask generation, providing different mask creation strategies.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- output_format
- Determines the output image format, affecting the visual presentation of the mask.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str

## Optional
- optional_roi_mask
- Optional region of interest mask, usable for further optimizing the mask generation process.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor

# Output types
- image
- Output is an image with the mask applied, demonstrating the result of the mask operation.
    - Comfy dtype: IMAGE
    - Python dtype: torch.Tensor
