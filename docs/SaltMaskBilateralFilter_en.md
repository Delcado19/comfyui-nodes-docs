...
SaltMaskBilateralFilter node applies bilateral filtering to masked regions, smoothing areas while preserving edges, improving mask region quality by reducing noise while maintaining structural integrity.

# Input types
## Required
- masks
    - Input mask to be processed with bilateral filtering, designed to smooth mask regions while preserving their edges.
    - Comfy dtype: MASK
    - Python dtype: List[torch.Tensor]
## Optional
- diameter
    - Specifies the diameter of each pixel neighborhood used during filtering.
    - Comfy dtype: INT
    - Python dtype: List[int] or int
- sigmaColor
    - Represents the filter sigma in color space, controlling the degree of color mixing within the neighborhood.
    - Comfy dtype: FLOAT
    - Python dtype: List[float] or float
- sigmaSpace
    - Determines the filter sigma in coordinate space, affecting the spatial proximity of pixels considered during filtering.
    - Comfy dtype: FLOAT
    - Python dtype: List[float] or float

# Output types
- MASKS
    - Output tensor of regions after applying bilateral filtering, where each region is smoothed while preserving edges.
    - Comfy dtype: MASK
