...
The RandomGeneratorForList node is designed to generate a sequence of random values based on a given seed and unique identifier. It ensures each sequence is unique to its identifier, enabling reproducible randomness across different executions.
...
- seed
    - The seed input determines the starting point for random number generation, ensuring reproducibility of the sequence.
    - Comfy dtype: INT
    - Python dtype: int

# Output types
- signal
    - The signal output passes through unchanged, serving as a continuity element in the node's operation.
    - Comfy dtype: *
    - Python dtype: utils.any_typ
- random_value
