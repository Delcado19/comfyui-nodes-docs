...
The Lora Loader Stack (rgthree) node aims to dynamically load multiple LoRA (Low-Rank Adaptation) modifications into a given model and CLIP, based on specified LoRA files and their associated strengths to enhance or alter their capabilities. It allows for sequential application of up to four LoRA modifications, providing a flexible mechanism for model customization.

# Input types
## Required
- model
    - The model parameter represents the neural network model to which LoRA modifications will be applied. It is crucial for defining the underlying architecture to be enhanced or altered by LoRA layers.
    - Comfy dtype: MODEL
    - Python dtype: torch.nn.Module
- clip
    - The clip parameter represents the CLIP model that will be adjusted using LoRA modifications. It plays a key role alongside the main model in specifying the target for LoRA enhancements.
    - Comfy dtype: CLIP
    - Python dtype: torch.nn.Module
- lora_i
    - Specifies the LoRA files to be applied to the model and CLIP. The choice of LoRA files directly influences the nature of the modifications, allowing for targeted adjustments.
    - Comfy dtype: COMBO[STRING]
    - Python dtype: str
- strength_i
    - Determines the strength of the LoRA modifications' impact on the model and CLIP, enabling fine control over the adaptation effect.
    - Comfy dtype: FLOAT
    - Python dtype: float
